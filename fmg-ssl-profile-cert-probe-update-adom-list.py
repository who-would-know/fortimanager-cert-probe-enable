...
# Gets a list of ADOMs from a file and save it to adomLIST
def get_adom_from_file():
    while True:
        # Ask user for filename
        print(f'Enter the filename with list of ADOMs: ')
        filename = input()
        ...
        if os.path.isfile(filename):
            ...
            print(f"File '{filename}' not found. Please try again, enter filename with list of ADOMs.\n")
...
def check_clone_profile(csADOM):
    ...
    for profile in sslPROFILE:    
        if profile == "certificate-inspection-std":
            return True
    return False
...
def update_cert_probe_failure(csADOM):
    profile = "certificate-inspection-std"
...
def clone_ssl_profile(csADOM):
    ...
    clone_profile_name = "certificate-inspection-std"
    ...
    print("\n")
    print ('--> Cloning SSL-SSH Profile certificate-inspection to %s with cert-probe-failure allow for ADOM %s' % (clone_profile_name, csADOM))
    print ('<-- Hcode: %d Jmesg: %s' % (r.status_code, json_resp['result'][0]['status']['message']))    
...
def main():
    ...
    match default_userOPTION:
        case "1":
            ...
            #Lock ADOM (skip if failed)
            if workspace_lock(adomNAME):
                ...
                print(f"<--SKIPPING {adomNAME}. Check to make sure it's unlocked")
                print ("\n")
                ERRORlog.write(f"\n<--SKIPPING {adomNAME}. Check to make sure it's unlocked")
        ...
        case "2":
            ...
            # get_adom(fgtDEVname)
            # print ("\n")
            # Display list imported from file
            print(f"<-- Following ADOMs will be processed:") 
